    ...
    return(
        <div className={styles.days}>
           ...
           {Array(get_number_of__days(month)).fill(null).map((e:any,index:number)=>{
            ...
            return(
            <div key={index} className={styles.day} style={(score==start || score==end)?{backgroundColor:style?.primary?style.primary:'black',color:'white',cursor:'pointer'}:(score>start && score<end)?{backgroundColor:style?.secoundry?style.secoundry:'#dbd8d8',color:style?.primary?style.primary:'black',cursor:'pointer'}:(start && !end && score>start && score<=hover)?{backgroundColor:style?.secoundry?style.secoundry:'#dbd8d8',color:style?.primary?style.primary:'black',cursor:'pointer'}:{cursor:(start && !end && hover<start)?'no-drop':'pointer'}} onMouseEnter={()=>{
                set_hover(score)
            }} onMouseOut={()=>{
                set_hover(0)
            }} onClick={()=>{
...
export default function RangeDatePicker({callback,start_default_value,end_default_value,style}:{style?:{primary?:string,secoundry?:string},callback:(e?:{start:string | null ,end:string | null})=>any,start_default_value:{year:number,month:number,day:number},end_default_value:{year:number,month:number,day:number}}){
    ...
    return(
        ...
        <div className={styles.container}>
            ...
            <Week_days/>
            <Days style={style} year={year} day={day} month={month} start={start_day} set_start={set_start} end={end_day} set_end={set_end} hover={hover_day} set_hover={set_hover}/>
            <div style={{display:'flex',marginTop:20,justifyContent:'space-between'}}>
                ...
                <div className={styles.apply_btn} style={{backgroundColor:style?.primary?style.primary:'black'}} onClick={()=>{
                  callback({
